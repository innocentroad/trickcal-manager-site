...
(function initAnnouncementHistoryData(root, factory) {
  ...
  const HISTORY = {
    ...
    entries: [
      { id: '20260926-fix-aside-combat-power', date: '2026-09-26', category: 'fix', title: 'アサイドのステータス・戦闘力計算を更新', items: ['追加調査に基づき、アサイドのステータス計算と戦闘力の計算式・端数処理を見直しました。従来と表示値が変わる場合があります。', 'ダメージ計算で敵使徒の育成設定を変更したときに、ステータスがずれる問題を修正しました。', '更新前の保存データを読み込んだ際の、計算値の更新処理を修正しました。', '旧保存データに再計算が必要と表示された場合は、ステータス管理で該当スロットの育成設定を確認し、不足項目を再設定して保存してください。'] },
